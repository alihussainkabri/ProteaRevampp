...
const ResetIMEI = ({ navigation }) => {
    ...
    async function login() {
        if (username && password) {
            ...
            const response = await fetch("https://" + defaultUrl + '/api/Login/SendOtp', {
                method: 'POST',
                headers: {
            ...
            if (response.ok == true) {
                ...
                if (data?.error_code == 200) {
                    ...
                    setTimeout(() => {
                        navigation.navigate('VerifyOTPIMEI', {
                            username,
                            password
                        })
                    }, 200);
                    ...
                    <VStack flex={1} justifyContent='space-between' height={Dimensions.get('window').height + 10}>
                        <View>
                            <ImageBackground source={require('../../assets/images/login-BG.png')} style={styles.titleBG} resizeMode='stretch'>
                                <Text fontFamily={fonts.PopB} fontSize={42} color='white'>Reset IMEI</Text>
                                <Text mt={-1} fontFamily={fonts.UrbanM} fontSize={16} color='white'>Please provide below details for reset your IMEI number.</Text>
                            </ImageBackground>
                            ...
                            <VStack px='20px' mt={16}>
                                ...
                                <VStack mb={9}>
                                    <Input InputLeftElement={<Image style={styles.icon} source={require('../../assets/icons/user.png')} />} placeholder="Employee ID" variant='unstyled' style={styles.input} borderBottomColor='gray.400' borderBottomWidth={2} value={username} onChangeText={setUsername} />
                                </VStack>
                        ...
                        <VStack>
                            <TouchableOpacity onPress={login} style={styles.btn}>
                                <Text fontFamily={fonts.PopSB} fontSize={20} color='white' textAlign='center'>Reset</Text>
                            </TouchableOpacity>
                        </VStack>
                    </VStack>
                </ScrollView>
